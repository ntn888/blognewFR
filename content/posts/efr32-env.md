...
```
sudo dnf install git git-lfs arm-none-eabi-gcc-cs make bear arm-none-eabi-newlib openocd
```

>On Fedora `arm-none-eabi-gdb` is not available in the repos. See [here](https://copr.fedorainfracloud.org/coprs/rleh/arm-none-eabi-gdb/) to install gdb.
...
[Simplicity commander CLI](https://docs.silabs.com/matter/latest/matter-references/flash-silabs-device) tool provided from Si Labs with J-Link. Or use [Blackmagic Probe](https://github.com/blackmagic-debug/blackmagic). Remember you can use the blackmagic tool over STlink or DAP adapter.
